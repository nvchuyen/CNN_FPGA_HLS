...
#define CONV2_BUFFER_SIZE (IMAGE_SIZE * IMAGE_CHANNELS * (CONV2_KERNEL_SIZE -1) + CONV2_KERNEL_SIZE * IMAGE_CHANNELS)
//typedef float  				float24_t;

void convolution(hls::stream<float24_t> &in, hls::stream<float24_t> &out,
		float24_t weights [CONV2_FILTERS][CONV2_CHANNELS][CONV2_KERNEL_SIZE][CONV2_KERNEL_SIZE],
		float24_t bias[CONV2_BIAS_SIZE]
		) {

	static int i =0, j =0, k =0, filter =0, filter_1, filter_2;
	...
	buffer<CONV2_BUFFER_SIZE> conv_buff;
#pragma HLS ARRAY_PARTITION variable = conv_buff complete

	/*
	 * Read the initial buffer
	 * */
	conv_init_buffer:for (i = 0; i < CONV2_BUFFER_SIZE; i++) {
		if (in.empty() == 0) {
			placeholder = in.read();
	...
	conv_layer2_label_stride:for (i = 0; i < (IMAGE_SIZE - CONV2_KERNEL_SIZE + 1); i += CONV2_STRIDE)
		conv_layer2_label9: for (j = 0;	j < (IMAGE_SIZE - CONV2_KERNEL_SIZE + 1); j += CONV2_STRIDE)
		{
#pragma HLS PIPELINE II=32
			conv_layer1_label2: for (filter = 0; filter < 1; filter++)
			{
#pragma HLS UNROLL
				sum = 0;
				conv_layer2_label6: for (row_offset = 0; row_offset < CONV2_KERNEL_SIZE; row_offset++)
				{
#pragma HLS UNROLL
					conv_layer2_label7: for (col_offset = 0; col_offset < CONV2_KERNEL_SIZE; col_offset++)
					{
#pragma HLS UNROLL
//						conv_layer2_label8: for (channel_offset = 0; channel_offset < CONV2_CHANNELS; channel_offset++)
//						{
//#pragma HLS UNROLL
							t1 = row_offset * IMAGE_SIZE * IMAGE_CHANNELS;
							t2 = col_offset * IMAGE_CHANNELS;
							val1 = conv_buff.GetValue(t1 + t2 + channel_offset);
							val2 = weights[filter][1][row_offset][col_offset];
							sum += val1 * val2;
//						}
					}
				}
			...
			if ((j + CONV2_STRIDE < (IMAGE_SIZE - CONV2_KERNEL_SIZE + 1)))
			{
				conv_layer2_label1: for (p = 0; p < IMAGE_CHANNELS; p++)
					if (in.empty() == 0)
					{
						placeholder = in.read();
						conv_buff.InsertBack(placeholder);
			...
			else if ((i + CONV2_STRIDE < (IMAGE_SIZE - CONV2_KERNEL_SIZE + 1)) && (j + CONV2_STRIDE >= (IMAGE_SIZE - CONV2_KERNEL_SIZE + 1)))
				conv_layer2_label0: for (p = 0; p < CONV2_KERNEL_SIZE * IMAGE_CHANNELS; p++)
					if (in.empty() == 0)
					{
						placeholder  = in.read();
						conv_buff.InsertBack(placeholder);
...
//
//void convolution(hls::stream<float24_t> &in, hls::stream<float24_t> &out,
//		float24_t weights [CONV1_FILTERS][CONV1_CHANNELS][CONV1_KERNEL_SIZE][CONV1_KERNEL_SIZE],
